...
const SmoothLineChart = ({ data, width, height, onPointPress }) => {
    ...
    const padding = { left: 35, right: 15, top: 30, bottom: 50 };
    ...
    return (
        <Svg width={width} height={height}>
            ...
            {/* Grid lines - only 0, 50, 100 */}
            {[0, 50, 100].map(val => {
                const y = padding.top + chartHeight - ((val - minScore) / (maxScore - minScore)) * chartHeight;
                return (
                    <G key={val}>
                        <SvgLine x1={padding.left} y1={y} x2={width - padding.right} y2={y} stroke="#E5E7EB" strokeWidth="1" strokeDasharray="4,4" />
                        <SvgText x={padding.left - 8} y={y + 4} fontSize="10" fill="#9CA3AF" textAnchor="end">{val}</SvgText>
                    </G>
                );
            })}
            ...
            {avgScore && (() => {
                ...
                return <SvgLine x1={padding.left} y1={avgY} x2={width - padding.right} y2={avgY} stroke="#22C55E" strokeWidth="2" strokeDasharray="6,6" opacity="0.5" />;
            })()}

            {/* Smooth line */}
            {pathData && <Path d={pathData} stroke="url(#lineGradient)" strokeWidth="3" fill="none" strokeLinecap="round" strokeLinejoin="round" />}

            {/* Data points */}
            {points.map((point, i) => (
                <Circle key={i} cx={point.x} cy={point.y} r="6" fill="#22C55E" stroke="#fff" strokeWidth="2" onPress={() => onPointPress && onPointPress(data[i])} />
            ))}
            ...
            {points.map((point, i) => {
                if (i % Math.ceil(data.length / 5) === 0 || i === data.length - 1) {
                    return (
                        <SvgText key={`label-${i}`} x={point.x} y={height - 10} fontSize="9" fill="#6B7280" textAnchor="middle">
...
export const NativeMoodScoreChart = ({ data = [], width = SCREEN_WIDTH - 48 }) => {
    ...
    const chartData = useMemo(() => {
        ...
        return sorted.map(entry => {
            const score = entry.situations && entry.situations.length
                ? Math.round(entry.situations.reduce((sum, s) => sum + calculateNormalizedScore(s), 0) / entry.situations.length)
                : 50;
    ...
    const uniqueSituations = useMemo(() => {
        ...
        return [...situationSet].map((name, i) => ({ name, color: TAG_COLORS[i % TAG_COLORS.length] }));
    }, [data]);

    const avgScore = chartData.length > 0 ? Math.round(chartData.reduce((sum, d) => sum + d.score, 0) / chartData.length) : 0;

    // Calculate dynamic width for scrolling - 6 dots per screen
    const pointWidth = (width / 6); // 6 points per screen width
    const minChartWidth = width;
    const dynamicWidth = Math.max(chartData.length * pointWidth, minChartWidth);
    ...
    return (
        <View style={styles.card}>
            ...
            {/* Scrollable Chart */}
            <ScrollView horizontal showsHorizontalScrollIndicator={true} style={{ marginVertical: 12 }}>
                <SmoothLineChart data={chartData} width={dynamicWidth} height={220} onPointPress={setSelectedPoint} />
            </ScrollView>
                ...
                <>
                    ...
                    <View style={styles.tagsContainer}>
                        {uniqueSituations.map((tag, i) => (
                            <View key={i} style={[styles.tag, { backgroundColor: tag.color.bg, borderColor: tag.color.border }]}>
                                <Text style={[styles.tagText, { color: tag.color.text }]}>{tag.name}</Text>
                            </View>
                        ))}
                    </View>
                </>
                                            ...
                                            {selectedPoint.situations.map((s, i) => (
                                                <View key={i} style={styles.situationItem}>
                                                    ...
                                                    {s.emotions && s.emotions.length > 0 && (
                                                        <Text style={styles.emotionText}>{s.emotions.map(e => e.emotion).join(', ')}</Text>
                                                    )}
                                                </View>
                                            ))}
                                        </>
...
const styles = StyleSheet.create({
    card: { backgroundColor: "#fff", borderRadius: 16, padding: 16, marginBottom: 16, elevation: 1, shadowColor: "#000", shadowOpacity: 0.04, shadowRadius: 6, borderWidth: 1, borderColor: "#F3F4F6" },
    header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 },
    title: { fontSize: 16, fontWeight: "700", color: "#1F2937" },
    avgBadge: { backgroundColor: "#DCFCE7", paddingHorizontal: 10, paddingVertical: 5, borderRadius: 16 },
    avgText: { fontSize: 11, fontWeight: "600", color: "#166534" },
    divider: { height: 1, backgroundColor: "#E5E7EB", marginBottom: 12 },
    sectionTitle: { fontSize: 13, fontWeight: "600", color: "#374151", marginBottom: 10, marginTop: 8 },
    tagsContainer: { flexDirection: 'row', flexWrap: 'wrap', gap: 6 },
    tag: { paddingHorizontal: 10, paddingVertical: 5, borderRadius: 16, borderWidth: 1 },
    tagText: { fontSize: 11, fontWeight: "500" },
    emptyText: { color: "#9CA3AF", fontSize: 13 },
    modalOverlay: { flex: 1, backgroundColor: "rgba(0, 0, 0, 0.5)", justifyContent: "center", alignItems: "center" },
    modalContent: { backgroundColor: "#fff", borderRadius: 20, padding: 20, width: "85%", maxWidth: 360, maxHeight: "80%", elevation: 5 },
    ...
    scoreValue: { color: "#22C55E", fontSize: 14 },
    situationsTitle: { fontSize: 14, fontWeight: "600", color: "#374151", marginTop: 12, marginBottom: 8 },
    situationItem: { marginBottom: 8, paddingLeft: 8 },
    situationName: { fontSize: 13, color: "#1F2937", fontWeight: "500" },
    emotionText: { fontSize: 11, color: "#6B7280", marginTop: 2, paddingLeft: 12 }
});
